...
- 적 type A는 특정 위치에 스폰 후 특정 위치까지 이동한 뒤 플레이어를 향해 사격한다.
- 적 type A는 HP, 총알 공격력, 연사 속도, 이동 속도, 충돌시 공격력을 갖는다.
- 적 type B는 특정 위치에 스폰 후 플레이어 방향으로 돌격해 자폭한다.
- 적 type B가 플레이어를 지나쳐 맵 밖(아래)으로 사라지면 position이 재설정돼 계속해서 돌격한다.
- 적 type B는 HP, 이동 속도, 충돌시 공격력을 갖는다.
- 적이 파괴될 때 폭파 애니메이션이 재생된다.
- 적이 파괴될 때 폭파 효과음이 재생된다.
...
- 적은 종류별로 다른 체력, 공격력, 공격속도, 이동속도를 가진다.
- 특정 적은 특수능력을 갖는다.
...
- 플레이어는 최대 이동속도 수치에 제약이 있다.
- 플레이어의 경험치가 일정 수치에 도달하면 레벨업한다.
- 플레이어의 경험치 요구량을 레벨에 따라 다르다.
- 플레이어는 웨이브가 끝날 때 레벨업한다.
...
- 플레이어는 상점에서 재화를 통해 기본 스탯을 상승시킬 수 있다.
- 기본 스탯에는 최대 HP, 공격력이 있다.
